...
func (r runContext) executeCall(kind tosca.CallKind, parameters tosca.CallParameters) (tosca.CallResult, error) {
	...
	if parameters.CodeAddress == (tosca.Address{}) && parameters.Recipient != (tosca.Address{}) {
		parameters.CodeAddress = parameters.Recipient
	}
	...
	if kind == tosca.Call && r.blockParameters.Revision >= tosca.R09_Berlin && !isPrecompiled &&
		!isStateContract && !r.AccountExists(parameters.CodeAddress) && parameters.Value.Cmp(tosca.Value{}) == 0 {
